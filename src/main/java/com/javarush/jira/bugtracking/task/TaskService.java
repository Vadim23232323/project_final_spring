...
public class TaskService {
    ...
    public void addTag(long taskId, String tag) {
        ...
        if (task.getTags().contains(tag)) {
            throw new DataConflictException("Task already has this tag");
        }
    ...
    public void removeTag(long taskId, String tag) {
        Task task = handler.getRepository().getExisted(taskId);
        if (!task.getTags().contains(tag)) {
            throw new NotFoundException("Task doesn't have this tag");
        }
        task.getTags().remove(tag);
        handler.getRepository().save(task);
    }

    // Методы для подсчета времени в статусах
    public Duration getWorkTime(Task task) {
        List<Activity> activities = activityHandler.getRepository().findAllByTaskIdOrderByUpdatedDesc(task.id());
        LocalDateTime inProgressStarted = null;
        LocalDateTime readyForReviewEnded = null;

        for (Activity activity : activities) {
            if (activity.getStatusCode().equals("in_progress")) {
                inProgressStarted = activity.getUpdated();
            } else if (activity.getStatusCode().equals("ready_for_review")) {
                readyForReviewEnded = activity.getUpdated();
            }
        }

        if (inProgressStarted != null && readyForReviewEnded != null) {
            return Duration.between(inProgressStarted, readyForReviewEnded);
        }
    ...
    public Duration getTestingTime(Task task) {
        List<Activity> activities = activityHandler.getRepository().findAllByTaskIdOrderByUpdatedDesc(task.id());
        LocalDateTime readyForReviewStarted = null;
        LocalDateTime doneEnded = null;

        for (Activity activity : activities) {
            if (activity.getStatusCode().equals("ready_for_review")) {
                readyForReviewStarted = activity.getUpdated();
            } else if (activity.getStatusCode().equals("done")) {
                doneEnded = activity.getUpdated();
            }
        }

        if (readyForReviewStarted != null && doneEnded != null) {
            return Duration.between(readyForReviewStarted, doneEnded);
        }
